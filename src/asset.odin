...
load_pixels :: proc(texture_file: string) -> (pixels: []byte, size: [2]u32) {
	...
	img, err := image.load_from_file(texture_path, {.alpha_add_if_missing}, context.temp_allocator)
	pixels_byte_size := img.width * img.height * img.channels

	pixels = img.pixels.buf[:]
	size = {u32(img.width), u32(img.height)}
...
load_cubemap_texture_files :: proc(
	copy_pass: ^sdl.GPUCopyPass,
	texture_files: [sdl.GPUCubeMapFace]string,
) -> ^sdl.GPUTexture {
...
load_gltf_file :: proc(copy_pass: ^sdl.GPUCopyPass, mesh_file: string) -> Mesh {
	mesh_path := filepath.join({CONTENT_DIR, "meshes", mesh_file}, context.temp_allocator)
	vertices, indices := load_gltf_model(mesh_path)
