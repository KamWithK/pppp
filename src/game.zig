...
pub fn init(
    ...
    const state = allocator.create(AppState) catch return .failure;
    state.* = .{
        .device = device,
        .window = window,
        .pipeline = pipeline,
...
pub fn iterate(app_state: *AppState) !sdl3.AppResult {
    ...
    app_state.device.unmapTransferBuffer(app_state.instance_transfer_buffer);

    const command_buffer = app_state.device.acquireCommandBuffer() catch return .failure;

    const copy_pass = command_buffer.beginCopyPass();
    copy_pass.uploadToBuffer(
        .{
            .transfer_buffer = app_state.instance_transfer_buffer,
            .offset = 0,
        },
        .{
            .buffer = app_state.instance_buffer,
            .offset = 0,
            .size = sprite_data_size,
        },
        true,
    ...
    const swapchain_texture = command_buffer.waitAndAcquireSwapchainTexture(app_state.window) catch return .failure;
    ...
    if (swapchain_texture.texture) |texture| {
        ...
        render_pass.bindVertexStorageBuffers(
            0,
            &.{app_state.instance_buffer},
        );
        render_pass.bindFragmentSamplers(
            0,
            &.{.{
                .texture = app_state.atlas_texture,
