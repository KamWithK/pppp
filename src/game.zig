...
pub fn iterate(app_state: *AppState) !sdl3.AppResult {
    const command_buffer = try app_state.device.acquireCommandBuffer();
    const swapchain_texture = try command_buffer.waitAndAcquireSwapchainTexture(app_state.window);
    ...
    if (swapchain_texture.texture) |texture| {
        ...
        render_pass.bindFragmentSamplers(
            0,
            &.{.{
                .texture = app_state.image_texture,
