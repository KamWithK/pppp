...
var game_init: ?*const fn (game_context: *?*GameContext) callconv(.C) sdl3.AppResult = null;
var game_iterate: ?*const fn (game_context: *GameContext) callconv(.C) sdl3.AppResult = null;
var game_event: ?*const fn (game_context: *GameContext, curr_event: *const sdl3.events.Event) callconv(.C) sdl3.AppResult = null;
var game_quit: ?*const fn (game_context: *GameContext, result: sdl3.AppResult) callconv(.C) void = null;
...
pub fn main() u8 {
    ...
    return sdl3.main_funcs.enterAppMainCallbacks(&args, GameContext, init, iterate, event, quit);
}
...
fn init(
    ...
    return (game_init orelse return .failure)(game_context);
}
...
fn iterate(
    ...
    const context = game_context orelse {
        std.debug.print("iterate: game_context is null\n", .{});
        return .failure;
    };
