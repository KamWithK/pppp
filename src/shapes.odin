...
generate_box_mesh :: proc(
	...
	num_vertices :=
		(get_plane_num_vertices(segments_x, segments_y) * 2 +
			get_plane_num_vertices(segments_x, segments_z) * 2 +
			get_plane_num_vertices(segments_z, segments_y) * 2) // front/back// top/bottom// left/right
	num_indices :=
		(get_plane_num_indices(segments_x, segments_y) * 2 +
			get_plane_num_indices(segments_x, segments_z) * 2 +
			get_plane_num_indices(segments_z, segments_y) * 2) // front/back// top/bottom// left/right
...
build_plane :: proc(
	...
	for _ in 0 ..= row.segments {
		...
		for _ in 0 ..= col.segments {
			add_vertex(ctx, {pos = col_pos, normal = normal, uv = col_uv, color = WHITE})
