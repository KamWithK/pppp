...
upload_mesh :: proc(
	...
	vertices: []$T,
	indices: []$S,
	extra_vertex_usage: sdl.GPUBufferUsageFlags = {},
	extra_index_usage: sdl.GPUBufferUsageFlags = {},
) -> Mesh {
...
upload_mesh_bytes :: proc(
	...
	vertex_buf := sdl.CreateGPUBuffer(
		g.gpu,
		{usage = extra_vertex_usage + {.VERTEX}, size = u32(vertices_byte_size)},
	)

	index_buf := sdl.CreateGPUBuffer(
		g.gpu,
		{usage = extra_index_usage + {.INDEX}, size = u32(indices_byte_size)},
	)
	...
	return {vertex_buf = vertex_buf, index_buf = index_buf, num_indices = u32(num_indices)}
}
