...
float3 blinnPhongBRDF(float3 dirToLight, float3 dirToView, float3 surfaceNormal,
                      float3 materialDiffuseReflection) {
  ...
  return materialDiffuseReflection +
         specularReflection; // TODO: energy conservation / normalization?
}
...
float4 main(Input input) : SV_Target0 {
  ...
  float incidenceAngleFactor =
      dot(dirToLight, surfaceNormal); // 1 direct incidence, 0 - no incidence,
                                      // -1 incidence from the other side
  if (incidenceAngleFactor > 0) {
    float attenuationFactor =
        1 / (distToLight * distToLight); // TODO: add more control variables
    float3 incomingRadiance = lightColor * lightIntensity;
    float3 irradiance =
        incomingRadiance * incidenceAngleFactor * attenuationFactor;
    float3 brdf = blinnPhongBRDF(dirToLight, dirToView, surfaceNormal,
