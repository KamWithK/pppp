...
Output main(Input input) {
  ...
  float2 vertexPosition = float2(
      -1 + float((input.vertexId & 1)
                 << 2), // for vertex 0 and 2 we get -1 + 0, because bit 1 isn't
                        // set. for vertex 1 we get -1 + 4, because 1 << 2 is 4
      -1 + float((input.vertexId & 2)
                 << 1) // for vertex 0 and 1 we get -1 + 0, because bit 2 isn't
                       // set. for vertex 2 we get -1 + 4, because 2 << 1 is 4
  );
